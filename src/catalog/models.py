...
class Artist(models.Model):
    ...
    # ---- Step 6: Computed filing properties ----
    @property
    def filing_artist(self) -> "Artist":
        """
        The artist we file under for sorting/binning:
        - If filed_under_artist is set, we use that artist's sort_name/alpha_bucket
        - Else we use self.
        """
    ...
    def save(self, *args, **kwargs):
        ...
        # If filed_under_artist is set, store that artist's sort_name/alpha_bucket
        # so DB ordering/binning uses the filed-under rule.
        filing = self.filed_under_artist
        if filing is not None:
            # ensure filing has the fields we need (in case it was deferred)
        ...
        # Save self first (so dependents can reference stable stored values)
        super().save(*args, **kwargs)

        # --- OPTIONAL: Cascade updates to artists that file under THIS artist ---
        # Keep "files_here" stored fields in sync when this artist changes.
...
class PhysicalBin(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["zone", "shelf_number", "bin_number"], name="uniq_physicalbin_zone_shelf_bin"),
        ]
    ...
    def active_mapping(self):
        """
        Returns the active BinMapping for this PhysicalBin, if any.

        The 'mappings' reverse relation comes from BinMapping.physical_bin(related_name="mappings").
        If someone ever changes that related_name, we fall back to the default *_set name.
        """
        mgr = getattr(self, "mappings", None)
        if mgr is None:
            mgr = getattr(self, "binmapping_set", None)
        if mgr is None:
            return None
    ...
    def effective_capacity(self) -> int:
        ...
        - If mapped to an active logical bin, use that logical bin's effective_capacity
        - Else fall back to zone default
        """
...
class RebinMove(models.Model):
    ...
    old_logical_bin = models.ForeignKey("LogicalBin", on_delete=models.SET_NULL, null=True, blank=True, related_name="+")
    new_logical_bin = models.ForeignKey("LogicalBin", on_delete=models.SET_NULL, null=True, blank=True, related_name="+")
